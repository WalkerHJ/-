...
import { Spin, message, Avatar, DatePicker, Progress, Input, Slider, Modal, Tooltip } from 'antd'
...
import TaskMenu from '../fourQuadrant/components/TaskMenu'
...
const WorkPlanMind = () => {
  ...
  const initFn = () => {
    ...
    console.log($store.getState().mindMapData)
  }
  /**
   * 监听窗口显示时初始化加载
   */
  ...
  const WorkPlanMindTitleData = (resData: any, type: string) => {
    ...
    state.mindTitleData.map((item: any, index: number) => {
      if (item.id == resData.id && item.typeId == resData.typeId) {
        isRun = false
      }
  ...
  //脑图右键
  const rightClick = () => {
    jQuery('jmnode').contextmenu(function(e) {
  ...
  //*****************************************局部刷新*****************************************//
  const refreshNodeMap = (type: any, nodeData: any) => {
