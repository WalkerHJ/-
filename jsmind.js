...
; (function ($w) {
    ...
    console.log('jsmind-------------------------')
    //公共变量
    let isremove = false //是否能删除
    var inputQriginalText = '' //缓存编辑文本内容
    let isMousedown = false //解决按下与单击冲突
    var isClickAddTask = false //是否为添加上下左右节点状态下
    var taskTypeH = 36; //计算节点位置（jmnode节点高度的一半[因为从新绘制了线]）
    ...
    var $h = function (n, t, type) {
        jQuery('.add_task').hide()
        if(type == 'tsx'){
            ReactDOM.render(t, document.getElementById(n.id));
        }else{
    ...
    jm.prototype = {
        ...
        mousedown_handle: function (e) {
            ...
            if (!!nodeid) {
                ...
                // 2:右键都可以选中
                // 3:点击时间插件可以选中
                if(element.parentNode.className == 'add_task'){
                    return
                }
                this.select_node(nodeid);
                jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',nodeid)
            } else {
                this.select_clear();
                isClickAddTask = false
                jQuery('.eidt_Kr_Process').hide()
                jQuery('.portrait_okrMain,.add_task').hide()
            }
        },

        //清除界面DOM元素及组件
        clear_doms:function(datas){
            if(datas.name == 'showPicker'){ //清除时间组件
                let selecttimesList = document.getElementsByClassName(`showPicker`)[0]
                if(datas.type == 0){
                    if(selecttimesList){
                        let selecttimes = selecttimesList.classList
                        let showTimesId = ''
                        for(var i =0;i<=selecttimes.length;i++){
                            if(selecttimes[i].includes('times')){
                                showTimesId = selecttimes[i].split('times')[1]
                                break;
                            }
                        }
                        if(datas.nodeid != showTimesId){
                            selecttimesList.setAttribute('style', 'display: none')
                            selecttimesList.classList.remove("showPicker")
                        }
                    }
        ...
        click_handle: function (e) {
            var isBreaking = jQuery('.breaking_distribute').find('span').text() == '取消选择' //断点派发
            let isStaticmind = jQuery('#OKR_map_content').attr('data-pagetype') == 'staticmind' //是否是从工作台及任务管理点击
            if (!this.options.default_event_handle['enable_click_handle']) {
                return;
            }
            if(event.srcElement.className == 'jsmind-editor'){ //点击input 状态下不选中
                return
            }
            var element = e.target || event.srcElement;
            ...
            //权限s -----------------------------
            if (!!nodeid) {
                let taskFlag = this.get_node(nodeid).data.taskFlag //=2草稿任务 !=2派发后的任务
                var tsakAuth = this.get_node(nodeid).data.update //true有操作权限--任务
                var draftAuth = jQuery('.OKR_map_content').attr('data-auth') //'true'有操作权限--草稿
                var taskstatus = this.get_node(nodeid).data.taskStatus //2完成
                var taskflag = this.get_node(nodeid).data.taskFlag //3归档
                if(taskFlag == 2 && draftAuth=='false' && !isexpander){
                    //草稿权限
                    return
                }
                if(taskFlag != 2 && !tsakAuth && !isexpander){
                    //任务权限
                    tsakAuths = false
                }
                if((taskstatus == 2 || taskflag == 3) && !isexpander){
                    //完成及归档不能操作
                    return
                }
            }
             //权限e -----------------------------
            if((isBreaking && !isexpander) || (isStaticmind && !isexpander)){
                return
            }
            if (isexpander) {
                // var nodeid = this.view.get_binded_nodeid(element);
                ...
                return
            }else if((jQuery(e.target).hasClass('timeInput') || jQuery(e.target).parents('.timeInput').length > 0) && tsakAuths){ //点击时间
                isMousedown = true
                e.stopPropagation()
                let text = ''
                if(element.className.includes('okr_endTime')){
                    text = 'edit_endtimes'
                }else if(element.className.includes('okr_startTime')){
                    text = 'edit_starttimes'
                }
                jQuery('.optBtnMoreList,.portrait_okrMain,.eidt_Kr_Process').hide()
                jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',nodeid) //缓存节点数据
                // this.invoke_event_handle(jm.event_type.select, { evt: text, data: [e], node: nodeid });
            } else if((jQuery(e.target).hasClass('show_progress') || jQuery(e.target).parents('.show_progress').length > 0) && tsakAuths){ //点击进度
                jQuery('.optBtnMoreList,.eidt_Kr_Process').hide()
                let process = this.get_node(nodeid || newnode).data.process // 1私密 0正常
                if(jQuery(e.target).hasClass('show_progress')){
                    jQuery(e.target)
                    .find('input')
                    .show()
                    .focus()
                    .val(process)
                    .select()
                  jQuery(e.target).addClass('edit_show_progress')
                }
                isMousedown = true
                e.stopPropagation()
                jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',nodeid) //缓存节点数据
            }else if((jQuery(e.target).hasClass('confidence')) && tsakAuths){ //信心指数
                isMousedown = true
                jQuery('.optBtnMoreList,.portrait_okrMain').hide()
                e.stopPropagation()
                jQuery(e.target).parents('.okr_text_content ').find('.eidt_Kr_Process').show()
                jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',nodeid) //缓存节点数据
            }else if(jQuery(e.target).hasClass('edit_portrait') && tsakAuths){ //常用联系人
                isMousedown = true
                jQuery('.eidt_Kr_Process').hide()
                // e.stopPropagation()
                jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',nodeid) //缓存节点数据
            ...
            var _this = this
            if(!isMousedown){
                let newnode = _this.get_selected_node() ? _this.get_selected_node().id : ''
                if(nodeid || newnode){
                    //权限s -----------------------------
                    let taskFlag = this.get_node(nodeid || newnode).data.taskFlag //=2草稿任务 !=2派发后的任务
                    var tsakAuth = this.get_node(nodeid || newnode).data.update //true有操作权限--任务
                    var taskstatus = this.get_node(nodeid || newnode).data.taskStatus //2完成
                    var taskflag = this.get_node(nodeid || newnode).data.taskFlag //3归档
                    var draftAuth = jQuery('.OKR_map_content').attr('data-auth') //'true'有操作权限--草稿
                    if(draftAuth=='false'){
                        //草稿权限
                        return
                    ...
                    //     return
                    // }
                    if(taskstatus == 2 || taskflag == 3){
                        //完成及归档不能操作
                        return
                    }
                    //权限e -----------------------------
                    jQuery('.optBtnMoreList,.portrait_okrMain,.eidt_Kr_Process,.add_task').hide()
                    _this.view.confidenceadd_task(nodeid || newnode,element.className); //添加上下左右新增任务
                }
            }
        },
        ...
        dblclick_handle: function (e) {
            let isStaticmind = jQuery('#OKR_map_content').attr('data-pagetype') == 'staticmind' //是否是从工作台及任务管理点击
            var isBreaking = jQuery('.breaking_distribute').find('span').text() == '取消选择' //断点派发
            let affiliation = jQuery(e.target).hasClass('okr_affiliation')
            let progress = jQuery(e.target).hasClass('ant-input')
            let portrait = jQuery(e.target).hasClass('edit_portrait')
            let mindmap_datepickr = jQuery(e.target).hasClass('timeInput')
            if (!this.options.default_event_handle['enable_dblclick_handle']) {
                return;
            }
            if(e.srcElement.localName == 'jmexpander' || isStaticmind || isBreaking || affiliation || progress || portrait || mindmap_datepickr){
                return;
            }
        ...
        _show: function (mind) {
            ...
            new Promise(resolve => { //解决JSX语法导致的异步问题
                this.view.load();
                resolve('true')
              }).then(res => {
                if(res == 'true'){
        ...
        add_node: function (parent_node, nodeid, topic, data) { //新增子节点
            if (this.get_editable()) {
        ...
        insert_node_parent: function(node_before, nodeid, topic, data){ //添加父节点
            if (this.get_editable()) {
                ...
                if (!!node) {
                    ...
                    let _beforeid = '_last_'
                    this.mind.move_node(node_before.id, _beforeid, nodeid, 1); //移动节点
                    this.view.update_node(node,'tsx');
                    this.layout.layout();
                    this.view.show(false);
                    //自动选中为编辑状态
                    //this.select_node(nodeid);
                    //this.begin_edit(nodeid);
        ...
        insert_node_before: function (node_before, nodeid, topic, data) { //添加同级节点(上)
            if (this.get_editable()) {
                var beforeid = jm.util.is_node(node_before) ? node_before.id : node_before;
                var node = this.mind.insert_node_before(node_before, nodeid, topic, data);
                if (!!node) {
        ...
        insert_node_after: function (node_after, nodeid, topic, data) { //添加同级节点(下)
            if (this.get_editable()) {
        ...
        //节点更新
        update_node: function (nodeid, topic, type) {
            jQuery('.add_task').hide()
            if (this.get_editable()) {
                ...
                if (!!node) {
                    if (node.topic === topic && type != 'tsx') {
                        logger.info('nothing changed');
                        this.view.update_node(node, type);
                        return;
                    }
                    node.topic = topic;
                    this.view.update_node(node, type);
        ...
        //nodeid：拖动当前Id  parentid：拖动到的id(父级) beforeid：拖动到的下一个元素id beforeIndex：拖动到得下一个元素
        //direction：方向 1:右侧 -1:左侧 beforePid：拖动前的id(父级)
        move_node: function (nodeid, beforeid, parentid, direction,beforeIndex,beforePid) { //拖动
            if (this.get_editable()) {
                var fromParentId = this.get_node(nodeid).parent.id
                this.invoke_event_handle(jm.event_type.edit, { evt: 'move_node', data: [nodeid, beforeid, parentid, direction,fromParentId,beforePid,beforeIndex], node: nodeid});
                return
                var node = this.mind.move_node(nodeid, beforeid, parentid, direction);
                if (!!node) {
                    // this.invoke_event_handle(jm.event_type.edit, { evt: 'move_node', data: [nodeid, beforeid, parentid, direction,fromParentId], node: nodeid});
                    this.view.update_node(node,'tsx');
                    this.layout.layout();
                    this.view.show(false);
                }
        ...
        select_node: function (node) {
            ...
            this.invoke_event_handle(jm.event_type.select, { evt: 'select_node', data: [], node: node.id,tagName:this.mind.selected._data.view.element});
        },
    ...
    jm.layout_provider.prototype = {
        ...
        _layout_offset_subnodes: function (nodes) {
            ...
            while (i--) {
                ...
                if(node.parent.isroot){ //根节点的svg曲线宽度间距扩大
                    layout_data.offset_x = (this.opts.hspace + 30) * layout_data.direction + pd.view.width * (pd.layout.direction + layout_data.direction) / 2;
                }else{
        ...
        get_node_point: function (node) {
            ...
            var p = {};
            if(jQuery(`jmnode[nodeid='${node.id}'`).length > 0 && view_data.height == 0){
                view_data.height = jQuery(`jmnode[nodeid='${node.id}'`).height()
                node._data.view.height = view_data.height
            }
            if(jQuery(`jmnode[nodeid='${node.id}'`).length > 0 && view_data.width == 0){
                view_data.width = jQuery(`jmnode[nodeid='${node.id}'`).width()
                node._data.view.width = view_data.width
            }
            p.x = offset_p.x + view_data.width * (node._data.layout.direction - 1) / 2;
            console.log(view_data.height)
        ...
        expand_node: function (node) {
            ...
            this.jm.invoke_event_handle(jm.event_type.show, { evt: 'expand_node', data: [], node: node.data.nodeId,type:node.data.typeId,expanded:node.expanded,children:node.children });
        },
        //收起
        collapse_node: function (node) {
            node.expanded = false;
            this.part_layout(node);
            this.set_visible(node.children, false);
            this.jm.invoke_event_handle(jm.event_type.show, { evt: 'collapse_node', data: [], node: node.data.nodeId,type:node.data.typeId,expanded:node.expanded });
        },
    ...
    jm.graph_canvas.prototype = {
        ...
        draw_line: function (pout, pin, offset, node) {
    ...
    jm.graph_svg.prototype = {
        ...
        draw_line: function (pout, pin, offset,node) {
            var line = jm.graph_svg.c('path'); //贝塞尔曲线
            // var line = jm.graph_svg.c('polyline'); //折线
            var status = node.data.status
            var taskType = node.data.taskType
            if(status == '4' || status == '5' || status == '3' || taskType == '3'){ //派发及KR
                line.setAttribute('stroke', '#ACCAF7');
                line.setAttribute('stroke-width', this.opts.line_width);
                line.setAttribute('stroke-typeid', node.data.typeId);
            }else{
                line.setAttribute('stroke', '#D7D7D9');
                line.setAttribute('stroke-width', this.opts.line_width);
                line.setAttribute('stroke-typeid', node.data.typeId);
            }
            line.setAttribute('fill', 'none'); //填充颜色
            this.lines.push(line);
            this.e_svg.appendChild(line);
            this._bezier_to(line, pin.x + offset.x, pin.y + offset.y, pout.x + offset.x, pout.y + offset.y,node);
        },
        ...
        _bezier_to: function (path, x1, y1, x2, y2,node) {
            var taskType = node.data.taskType
            if(taskType != '2' && taskType != '3'){ //任务状态下
                x1 = x1 + 3
                x2 = x2 - 10 + 12
            }else{ //KR曲线调整
                y1 = y1 - 35
            }
            //x1,y1起点(子节点的坐标:原理是从子节点作为起点开始画)x2,y2终点(根节点的坐标)
            if(node.parent.isroot){
                let x2s = jQuery(`jmnode[nodeid='${node.parent.id}'`)[0].offsetWidth / 2
                x2 = x2 + x2s - 15
                y2 = y2 - 35
                //贝塞尔曲线
                // path.setAttribute('d', 'M' + x1 + ' ' + y1 + ' C ' + (x1 + (x2 - x1) * 2 / 3) + ' ' + y1 + ', ' + x1 + ' ' + y2 + ', ' + x2 + ' ' + y2);
                path.setAttribute('d', 'M' + x1 + ' ' + y1 + ' S ' + (x1 + (x2 - x1) * 2 / 3) + ' ' + y1 + ', ' + (x2 - 3) + ' ' + y2);
            }else{
                if(node.parent.data.taskType == '3'){
                    path.setAttribute('d', 'M' + x1 + ' ' + y1 + ' S ' + (x1 + (x2 - x1) * 2 / 3) + ' ' + y1 + ', ' + (x2 - 3) + ' ' + (y2 - 35));
                }else{
                    path.setAttribute('d', 'M' + x1 + ' ' + y1 + ' S ' + (x1 + (x2 - x1) * 2 / 3) + ' ' + y1 + ', ' + (x2 - 3) + ' ' + y2);
                }
            }
            ...
            //折线
            // path.setAttribute('points', x1 + ' ' + y1 + ',' + x2 + ' ' + y1 + ',' + x2 + ' ' + y2);
        },
    ...
    jm.view_provider.prototype = {
        ...
        create_node_element: function (node, parent_node) {
            ...
            if (node.isroot) {
                d.id = 'root'
                node.data.taskType == 2 ? d.className = 'root root_o' : d.className = 'root';
                if(node.data.status == 4 || node.data.status == 5 || node.data.status == 3){ //派发及已完成
                    d.className += ' accomplish';
                }
                if(node.data.taskStatus == 2){ //完成
                    d.className += ' complete';
                }
            } else {
                var d_e = $c('jmexpander');
                if(node.data.taskType == '2' || node.data.taskType == '3'){ //okr
                    d.className = 'task_okr';
                }else{//节点为任务
                    d.className = 'task_node';
                }
                if(node.data != undefined && node.data != null && node.data != ''){
                    if(node.data.status == 5 || node.data.taskStatus == 2){ //完成
                        d.className += ' accomplish complete';
                    }else if(node.data.status == 4 || node.data.status == 3){ //派发
                        d.className += ' accomplish';
                    }
                }
                $t(d_e, '-');
                ...
                d.id = `task-${node.data.typeId || getnode.id.split('###')[1]}`
                view_data.expander = d_e;
            }
            //暂时屏蔽【父级任务下只显示父级的任务类型】
            // if (node.parent) {
            //     if (node.parent.data.taskType != 2 && node.parent.data.taskType != 3) {
            //         d.className += ' no-genre'
            //     }
            // }
            new Promise(resolve => {  //解决JSX语法导致的异步问题
                ...
                if(res == 'true'){
                    if (!!node.topic) {
                        if (this.opts.support_html) { //前台传的html对象为tsx语法
                            $h(d, node.topic,'tsx');
                        } else {
                            $t(d, node.topic);
                        }
        ...
        update_node: function (node, type) {
            var view_data = node._data.view;
            var element = view_data.element;
            if (!!node.topic) {
                if (this.opts.support_html) {
                    if(type == 'tsx'){
                        $h(element, node.topic, 'tsx'); //更新节点的时候也使用tsx语法渲染（为了融合使用antd组件内容）
                    }else{
        ...
        select_node: function (node) {
            if (!!this.selected_node) { //移除
                ...
                var objAddTask = document.getElementsByClassName('add_task')
                if(objAddTask.length > 0){
                    objAddTask[0].remove()
                }
            }
            if (!!node) {
          ...
          // 添加节点
          confidenceadd_task:function(node,className){
            node = this.jm.get_node(node)
            var view_data = node._data.view;
            var element = view_data.element;
            let rightL = element.offsetWidth - 12
            let status = node.data.status
            let isAddKR = 'style="display:block"'
            if(node.isroot){
                isAddKR = 'style="display:none"'
            }else{
                if(node.parent.data.taskType == 2 && !node.parent.isroot){//第二层且父级为O的状态下可以添加父级KR
                    isAddKR = 'style="display:none"'
                }
            }
            // 添加上下左右新增任务
            if(node.isroot){ //o
                if(node.data.taskType == 2){ //第一层为O的状态下不可以新增父级 共享点击下第一层也不增加父级
                    var dom=document.createElement('label');
                    dom.className='add_task';
                    dom.innerHTML=`<label class='add_task'>
                    <i class="add_task_right" style="left:${rightL}px"></i>
                    </label>`;
                    ...
                    dom.className='add_task';
                    dom.innerHTML=`<label class='add_task'>
                    <i class="add_task_left"></i>
                    <i class="add_task_right" style="left:${rightL}px"></i>
                    </label>`;
                    element.appendChild(dom);
                }
            }else if(node.data.taskType == 3){ //kr
                var dom=document.createElement('label');
                dom.className='add_task';
                dom.innerHTML=`<label class='add_task'>
                <i class="add_task_right" style="left:${rightL}px"></i>
                <i class="add_task_top"></i>
                <i class="add_task_bottom"></i>
                </label>`;
                element.appendChild(dom);
            }else{
                var dom=document.createElement('label');
                dom.className='add_task';
                dom.innerHTML=`<i class="add_task_left" ${isAddKR}"></i>
                <i class="add_task_right" style="left:${rightL}px"></i>
                <i class="add_task_top"></i>
                <i class="add_task_bottom"></i>`;
                element.appendChild(dom);
            }
            if(className == 'add_task_left'){
                this.jm.invoke_event_handle(jm.event_type.select, { evt: 'add_left', data: [], node: node.id });
            }else if(className == 'add_task_right'){
                this.jm.invoke_event_handle(jm.event_type.select, { evt: 'add_right', data: [], node: node.id });
            }else if(className == 'add_task_top'){
                this.jm.invoke_event_handle(jm.event_type.select, { evt: 'add_top', data: [], node: node.id });
            }else if(className == 'add_task_bottom'){
                this.jm.invoke_event_handle(jm.event_type.select, { evt: 'add_bottom', data: [], node: node.id });
            }
        },
        ...
        edit_node_begin: function (node) {
            ...
            jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',node.id) //缓存节点数据
            ...
            var view_data = node._data.view;
            var element = view_data.element;
            // var topic = node.data.names;
            var topic = $(element).find('.okr_task_name').text()
            ...
            inputQriginalText = $(element).html()
            // element.innerHTML = '';
            ReactDOM.render('', document.getElementById(element.id)); //解决tsx语法清空innerHTML会导致内容render不正确问题
        ...
        edit_node_end: function () {
            if (this.editing_node != null) {
                ...
                jQuery('.okrnode').empty().attr('nodeid','').attr('nodeid',node.id) //缓存节点数据
                this.jm.invoke_event_handle(jm.event_type.select, { evt: 'edit_text', data: [],node: node.data.nodeId,type:node.data.typeId,topic:topic });
                // return
                ...
                //     }
                // } else {
                //     this.jm.update_node(node.id, topic,'tsx');
                // }
            }
        },
        ...
        _show: function () { //视图大小
            ...
            //渲染脑图记录上一次放大缩小
            const scalenum = jQuery('.scale-zoom i').text().split('%')[0]
            const scale = scalenum / 100
            this.setZoom(scale, jQuery('.scale-zoom i'),1)
        },

        zoomIn: function (obj) {
            return this.setZoom(this.actualZoom + this.zoomStep,obj);
        },
        ...
        setZoom: function (zoom,obj,type) {
            let newZoom = Number(zoom.toFixed(1))
            ...
            if(obj != ''){
                obj.text(`${parseInt(newZoom*100)}%`) //设置比例值
            }
            if(type != 1){
                this.show(true);
            }
        ...
        show_nodes: function () {
            ...
            for (var nodeid in nodes) {
                ...
                view_data = node._data.view;
                for(var i=0;i<node.children.length;i++){
                    if(node.children[i].data.status == 4 || node.children[i].data.status == 3 || node.children[i].data.status == 5){
                        _status = 4 //派发
                        break;
                    }    
                }
                ...
                node_element.style.top = ((_offset.y + p.y) - taskTypeH) + 'px';
                // if(node.parent){
                //     if(node.parent.data.taskType == '3'){ //Kr下面子节点
                //         node_element.style.top = ((_offset.y + p.y) - (taskTypeH * 2)) + 'px';
                //     }
                // }
                node_element.style.display = '';
                node_element.style.visibility = 'visible';
                if (!node.isroot && node.children.length > 0) {//加减号计算位置
                    expander_text = node.expanded ? '-' : '+';
                    p_expander = this.layout.get_expander_point(node);
                    expander.style.left = (_offset.x + p_expander.x) + 3 + 'px';
                    expander.style.top = (_offset.y + p_expander.y) + 'px';
                    expander.className = node.expanded ? 'jian' : 'jia';
                    if(node.data.taskType == '3'){ //Kr加减号保持在节点中间
                        expander.style.top = ((_offset.y + p_expander.y) - taskTypeH ) + 'px';
                    }
                    expander.style.display = '';
                    expander.style.visibility = 'visible';
                    $t(expander, expander_text);
                }else if(node.data.hasChild > 0 && node.children.length == 0){//判断是否有子任务 (加减号)
                    expander_text = '+'
                    p_expander= this.layout.get_expander_point(node);
                    expander.style.left = (_offset.x + p_expander.x) + 3 + 'px';
                    expander.style.top = (_offset.y + p_expander.y) + 'px';
                    expander.className = 'jia';
                    if(node.data.taskType == '3'){ //Kr加减号保持在节点中间
                        expander.style.top = ((_offset.y + p_expander.y) - taskTypeH ) + 'px';
                    }
                    expander.style.display = '';
                ...
                if (!node.isroot && node.children.length == 0 && node.data.hasChild == 0) {
                    expander.style.display = 'none';
    ...
    jm.shortcut_provider.prototype = {
        ...
        handle_delnode: function (_jm, e) {
            ...
            if (!!selected_node && !selected_node.isroot) {
                if(isremove){
                    this.jm.invoke_event_handle(jm.event_type.edit, { evt: 'remove_node', data: [selected_node.id], node: selected_node.data.nodeId,type:selected_node.data.typeId,topic:selected_node.topic });
                    isremove = false
                    return
