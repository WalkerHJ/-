 ...
 */
/* eslint-disable */ //声明eslint-disable忽略eslint检查
// 避免$冲突 修改为_$ __$jsMind
;(function($w) {
  'use strict'
  var $d = $w.document
  var __name__ = '__$jsMind'
  ...
  jsMind.draggable = function(jm) {
    ...
    this.shadow = null //拖动的dom元素
    this.before_node = null
    this.shadow_w = 0
    this.shadow_h = 0
  ...
  jsMind.draggable.prototype = {
    init: function() {
      jQuery('.jmdrag,.jmnodedrag').remove()
      this._create_canvas()
      this._create_shadow()
      this._event_bind()
    },
    ...
    reset_shadow: function(el, type) {
      ...
      s.top = el.style.top
      if (type == 'custom') {
        jQuery(this.shadow).width(jQuery(el)[0].offsetWidth)
        jQuery(this.shadow).height(jQuery(el)[0].offsetHeight)
      } else {
    ...
    _magnet_shadow: function(node) {
      if (!!node) {
        ...
        this.canvas_ctx.strokeStyle = '#ACCAF7'
    ...
    _event_bind: function() {
      ...
      var nodar = jQuery('.pattern_card').hasClass('active')
      if (nodar) {
        //四象限模式不能拖动
        return
      }
      ...
      jQuery('.dragItem')
        ...
        .on('mousedown', function(e) {
          // 右侧新增按住鼠标事件
          var evt = e || event
          jd.dragstartCustom.call(jd, evt, 'dragItem')
        })
      jQuery(container)
        .off('mousemove')
    ...
    dragstart: function(e) {
      let jminputEdit = jQuery('.jsmind-editor:visible').length //避免编辑冲突mousemove事件
      let jmmeetsEdit = jQuery('.meetings_okrMain:visible').length
      let jmrangeEdit = jQuery('.eidt_Kr_Process:visible').length
      let jmproEdit = jQuery('.edit_show_progress').length
      let targetDom = jQuery(event.target)
      const mindMaptype = $store.getState().mindMapData.form
      if (mindMaptype == 'workbench') {
        return
      }
      if (!this.jm.get_editable() || jminputEdit > 0 || jmmeetsEdit > 0 || jmrangeEdit > 0 || jmproEdit > 0) {
        return
      }
      if (
        targetDom.parent().hasClass('row_progress') ||
        targetDom.hasClass('row_progress') ||
        targetDom.parents('.timeInput').length > 0
      ) {
        //点击进度不执行拖动
        return
      }
      ...
      this.active_node = null
      this.before_node = null //拖拽开始时当前父节点

      var jview = this.jm.view
      var el = e.target || event.srcElement
      if (jQuery(el).parents('jmnode').length == 0 && jQuery(el)[0].nodeName != 'JMNODE') {
        return
      }
      if (jQuery(el).parents('jmnode').length > 0) {
        el = jQuery(el).parents('jmnode')[0]
      }
        ...
        if (!node.isroot) {
          ...
          this.hlookup_delay = $w.setTimeout(function() {
            ...
            jd.hlookup_timer = $w.setInterval(function() {
              jd.lookup_close_node.call(jd) //计算线条
    ...
    // 自定义开始拖动
    dragstartCustom: function(e, className) {
      if (this.capture) {
        return
      }
      this.active_node = null
      var el = e.target || event.srcElement
      if (jQuery(el).parents(`.${className}`).length > 0) {
        el = jQuery(el).parents(`.${className}`)[0]
      }
      var node = jQuery(el)
      if (!!node) {
        this.reset_shadow(el, 'custom')
        this.active_node = node
        let _left = jQuery('.OKR_map_content')[0].offsetLeft
        let _top = jQuery('.OKR_map_content')[0].offsetTop
        this.offset_x = 0 - 100 - _left
        this.offset_y = 0 + 100 + _top
        // this.offset_x = (e.clientX || e.touches[0].clientX) - el.offsetLeft;
        // this.offset_y = (e.clientY || e.touches[0].clientY) - el.offsetTop;
        this.client_hw = Math.floor(el.clientWidth / 2)
        this.client_hh = Math.floor(el.clientHeight / 2)
    ...
    drag: function(e) {
      ...
      let jmmeetsEdit = jQuery('.meetings_okrMain:visible').length
      let jmrangeEdit = jQuery('.eidt_Kr_Process:visible').length
      if (!this.jm.get_editable() || jminputEdit > 0 || jmmeetsEdit > 0 || jmrangeEdit > 0) {
        return
      }
      if (this.capture) {
        // console.log('mousemove---')
        ...
        this.shadow.style.left = px + 'px' //拖动节点的位置
        this.shadow.style.top = py + 'px'
        clear_selection()
        jQuery('.WorkPlanMindRightMeun .rightCon').addClass('hide_animate')
      }
    },
    ...
    move_node: function(src_node, target_node, target_direct) {
      ...
      if (!!target_node && !!src_node && !jsMind.node.inherited(src_node, target_node)) {
        ...
        if (src_node.id) {
          this.jm.move_node(src_node.id, beforeid, target_node.id, target_direct, node_before, before_target.id)
        } else if (src_node[0].className) {
          let _classNama = src_node[0].className
          if (_classNama.indexOf('dragItem') != -1) {
            //右侧拖动
            this.jm.move_node_offside(src_node, target_node, node_before, target_direct)
          }
        }
      }
      this.active_node = null
